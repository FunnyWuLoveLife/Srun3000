...
public class Connecter
{
	...
	public static String Login(User user)
	{
		...
		try
		{
			urlencode_username = URLEncoder.encode(user.getUsername(), "utf-8");// 制将字符串转换为
		...
		// 组建URL数据链
		String data = "action=login&username=" + urlencode_username
				+ "&password=" + urlencode_pwd
				+ "&drop=0&pop=1&type=2&n=117&mbytes=0&minutes=0&ac_id="
				+ urlencode_acid + "&mac=" + urlencode_mac;
	...
	public static String HttpPost(String url, String data) throws Exception
	{
		...
		/*
		 * 返回一个 URLConnection 对象，它表示到 URL 所引用的远程对象的连接。 每次调用此 URL 的协议处理程序的
		 * openConnection 方法都打开一个新的连接。
		 */
		URLConnection urlConnection = urlObj.openConnection();// 调用URL对象的openConnection
																// 方法打开一个新的连接
		urlConnection.setRequestProperty("Content-Type",
				"application/x-www-form-urlencoded");// 添加由键值对指定的一般请求属性。
		...
		httpURLConnection.setDoOutput(true);// 将DoOutput设置为true,这样就可以使用HttpURLConnectiono类的getOutputStream().write()
											// 方法
		byte[] contentBytes = data.getBytes("UTF-8");// 得到http类容的字节数组
		httpURLConnection.setFixedLengthStreamingMode(contentBytes.length);// 此方法用于在预先已知内容长度时启用没有进行内部缓冲的
																			// HTTP
																			// 请求正文的流。
